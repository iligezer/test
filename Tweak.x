...
BOOL safeWriteInt(uintptr_t addr, int value) {
    if (addr == 0) return NO;
    kern_return_t kr = vm_write(mach_task_self(), addr, (vm_offset_t)&value, 4);
...
BOOL safeWritePtr(uintptr_t addr, uintptr_t value) {
    if (addr == 0) return NO;
    kern_return_t kr = vm_write(mach_task_self(), addr, (vm_offset_t)&value, 8);
    return kr == KERN_SUCCESS;
}
...
uintptr_t callFunctionWithArg(uintptr_t addr, uintptr_t arg) {
    ...
    if (kr != KERN_SUCCESS) {
        addLogF(@"⚠️ CALL_FUNC_ARG: cannot get memory info for 0x%lx", addr);
        return 0;
    }
    
    if (!(info.protection & VM_PROT_EXECUTE)) {
        addLogF(@"⚠️ CALL_FUNC_ARG: address 0x%lx is not executable", addr);
        return 0;
    }
...
uintptr_t callFunctionWithTwoArgs(uintptr_t addr, uintptr_t arg1, uintptr_t arg2) {
    if (addr == 0) {
        addLogF(@"⚠️ CALL_FUNC_2: address is NULL");
        return 0;
    }
    ...
    if (kr != KERN_SUCCESS) {
        addLogF(@"⚠️ CALL_FUNC_2: cannot get memory info for 0x%lx", addr);
        return 0;
    }
    
    if (!(info.protection & VM_PROT_EXECUTE)) {
        addLogF(@"⚠️ CALL_FUNC_2: address 0x%lx is not executable", addr);
        return 0;
    }
    ...
    @try {
        ...
        addLogF(@"❌ Exception in CALL_FUNC_2 at 0x%lx: %@", addr, e);
        return 0;
    }
}

uintptr_t callFunctionWithThreeArgs(uintptr_t addr, uintptr_t arg1, uintptr_t arg2, uintptr_t arg3) {
    if (addr == 0) {
        addLogF(@"⚠️ CALL_FUNC_3: address is NULL");
        return 0;
    }
    ...
    if (kr != KERN_SUCCESS) {
        addLogF(@"⚠️ CALL_FUNC_3: cannot get memory info for 0x%lx", addr);
        return 0;
    }
    
    if (!(info.protection & VM_PROT_EXECUTE)) {
        addLogF(@"⚠️ CALL_FUNC_3: address 0x%lx is not executable", addr);
        return 0;
    }
    ...
    @try {
        ...
        addLogF(@"❌ Exception in CALL_FUNC_3 at 0x%lx: %@", addr, e);
        return 0;
    }
}
...
uintptr_t findSymbol(const char* symbolName) {
    ...
    addLogF(@"❌ Symbol %s not found", symbolName);
    return 0;
}
...
NSString* handleCommand(NSString *cmd) {
    @try {
        ...
        if (parts.count == 0) return @"ERROR: empty command";
        ...
        else if ([command isEqualToString:@"CALL_FUNC_ARG"]) {
        ...
        else if ([command isEqualToString:@"CALL_FUNC_2"]) {
            if (parts.count < 4) return @"ERROR: need addr, arg1, arg2";
            uintptr_t addr = strtoull([parts[1] UTF8String], NULL, 16);
        ...
        else if ([command isEqualToString:@"CALL_FUNC_3"]) {
            if (parts.count < 5) return @"ERROR: need addr, arg1, arg2, arg3";
            uintptr_t addr = strtoull([parts[1] UTF8String], NULL, 16);
            uintptr_t arg1 = strtoull([parts[2] UTF8String], NULL, 16);
            uintptr_t arg2 = strtoull([parts[3] UTF8String], NULL, 16);
            uintptr_t arg3 = strtoull([parts[4] UTF8String], NULL, 16);
            addLogF(@"📞 CALL_FUNC_3 0x%lx args 0x%lx,0x%lx,0x%lx", addr, arg1, arg2, arg3);
        ...
        addLogF(@"❌ CRASH in handleCommand: %@", e);
            ...
            @autoreleasepool {
                ...
                while (serverRunning) {
                    ...
                    @try {
                        ...
                        addLogF(@"❌ Error processing command: %@", e);
                        const char *errMsg = "ERROR: server error\n";
