...
// ========== ТВОИ RVA (ИЗ ТВОЕГО ЖЕ СКРИНШОТА) ==========
...
void* getRealPtr(uint64_t rva) {
    return (void*)(BASE_ADDR + rva - 0x1042c4000); // нормализация
}
...
void scanAndShowLog() {
    ...
    [self addLog:@"=== IL2CPP SCAN ==="];
    [self addLog:[NSString stringWithFormat:@"Base: 0x%llx", BASE_ADDR]];
    ...
    // TODO: Здесь можно добавить поиск игроков и вывод их данных
...
void addLog(NSString *text) {
    ...
    NSLog(@"%@", text); // дублируем в консоль
}

void showLogWindow() {
    UIWindow *logWindow = [[UIWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
void closeLogWindow() {
    [logTextView.window resignKeyWindow];
    logTextView.window.hidden = YES;
    logTextView = nil;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    // TODO: Здесь нужно получить список игроков и отрисовать их
    // Пока просто рисуем тестовую точку
